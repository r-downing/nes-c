...
CHIP8_ERROR chip8_load_rom_data(Chip8 *const c, const uint8_t *const rom_data, const uint16_t length)
{
...
CHIP8_ERROR chip8_load_instructions(Chip8 *const c, const uint16_t *const instructions, const uint16_t num_instructions)
{
...
{
    return fread(&c->mem[CHIP8_PROGRAM_START_ADDR], 1, sizeof(c->mem) - CHIP8_PROGRAM_START_ADDR, f);
}
...
CHIP8_ERROR chip8_cycle(Chip8 *const c)
{
    ...
    const Chip8Opcode op = {
        .u16 = (c->mem[c->pc]) << 8 | (c->mem[c->pc + 1])};
    c->pc += sizeof(op.u16);
