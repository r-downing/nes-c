...
bool init(void)
{
    ...
    window = SDL_CreateWindow("emscripten chip8", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
    if (!window)
    {
...
void handle_events()
{
    ...
    while (SDL_PollEvent(&e) != 0)
    {
        if (e.type == SDL_QUIT)
        {
            ...
            for(size_t i = 0; i < sizeof(chip8_keys) / sizeof(chip8_keys[0]); i++) {
